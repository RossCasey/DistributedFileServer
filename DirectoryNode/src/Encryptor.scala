...
object Encryptor {
  ...
  /**
   * Encrypts an error so that it is suitable for secure transmission
   *
   * @param error - error to encrypt
   * @param ticket - ticket to accompany encrypted error
   * @param key - key with which to encrypt error
   * @return ServerMessage that has been encrypted and is suitable for secure transmission
   */
